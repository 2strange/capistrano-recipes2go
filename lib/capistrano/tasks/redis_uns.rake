    ...
    task script.to_sym do
      on roles(fetch(:redis_uns_roles)) do
        ...
        within current_path do
          execute %(cd #{current_path} && REDIS_NAMESPACE=#{redis_namespace} REDIS_SOURCE_CONFIG='#{source_config_json}' REDIS_TARGET_CONFIG='#{target_config_json}' #{ruby_command} #{script_remote_path})
        end
      end
    end
  end
    ...
    ['old', 'new'].each do |rds|
      ...
      task "#{script}_#{rds}".to_sym do
        on roles(fetch(:redis_uns_roles)) do
          ...
          # Skript mit Umgebungsvariablen ausführen
          within current_path do
            execute %(cd #{current_path} && REDIS_UNS_CONFIG='#{uns_config_json}' #{ruby_command} #{script_remote_path})
          end
        end
      end
